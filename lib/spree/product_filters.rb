  ...
  module Core
    ...
    module ProductFilters
      ...
      if Spree::Property.table_exists?
        Spree::Product.add_search_scope :ingredient_any do |*opts|
          conds = opts.map {|o| ProductFilters.ingredient_filter[:conds][o]}.reject {|c| c.nil?}
          scope = conds.shift
          conds.each do |new_scope|
            scope = scope.or(new_scope)
          end
          Spree::Product.with_property("ingredient").where(scope)
        end

        def ProductFilters.ingredient_filter
          ingredient_property = Spree::Property.find_by_name("ingredient")
          ingredients = Spree::ProductProperty.where(:property_id => ingredient_property).pluck(:value).uniq
          pp = Spree::ProductProperty.arel_table
          conds = Hash[*ingredients.map { |b| [b, pp[:value].eq(b)] }.flatten]
          { :name   => "ingredient",
            :scope  => :ingredient_any,
            :conds  => conds,
            :labels => (ingredients.sort).map { |k| [k, k] }
          }
        end
      end
      ...
      if Spree::Property.table_exists?
        Spree::Product.add_search_scope :selective_ingredient_any do |*opts|
          Spree::Product.ingredient_any(*opts)
        end

        def ProductFilters.selective_ingredient_filter(taxon = nil)
          taxon ||= Spree::Taxonomy.first.root
          ingredient_property = Spree::Property.find_by_name("ingredient")
          scope = Spree::ProductProperty.where(:property_id => ingredient_property).
                                         joins(:product => :taxons).
                                         where("#{Spree::Taxon.table_name}.id" => [taxon] + taxon.descendants).
                                         scoped
          ingredients = scope.pluck(:value).uniq
          {
            :name   => "Ingredients",
            :scope  => :selective_ingredient_any,
            :labels => ingredients.sort.map { |k| [k,k] }
          }
        end
      end
